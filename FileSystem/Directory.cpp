	...
	namespace FileSystem
	{
        ...
        static void enumerateEntries(std::string const& path, std::string const& pattern, bool recursive, vector<File>* files, vector<Directory>* directories)
        {
			...
			if (fd != INVALID_HANDLE_VALUE)
			{
                ...
                {
                    std::string entryPath(Path::Combine(path, data.cFileName));
        ...
        static void enumerateEntries(std::string const& path, std::string const& pattern, bool recursive, vector<File>* files, vector<Directory>* directories)
        {
        	...
        	if (!dir)
        		throw std::runtime_error("Error reading directory");
        ...
        void Directory::erase()
        {
            ...
            if (rv)
            	throw std::runtime_error("Could not erase directory");
#endif
        }
        void Directory::move(std::string const& to, bool overwrite)
        {
            this->assertValid();
            ...
            {
            	if (!overwrite)
            		throw std::runtime_error("Target directory exists");
            }
            else
        ...
        void Directory::copy(std::string const& to, bool overwrite)
        {
            this->assertValid();
        ...
        vector<std::shared_ptr<FileSystemEntry> > Directory::enumerateChildren(std::string const& pattern, bool recursive) const
        {
            vector<std::shared_ptr<FileSystemEntry> > rv;
            ...
            for (vector<File>::iterator it = files.begin(); it != files.end(); it++)
                rv.push_back(std::make_shared<File>(*it));
            for (vector<Directory>::iterator it = dirs.begin(); it != dirs.end(); it++)
                rv.push_back(std::make_shared<Directory>(*it));

            return rv;
        }
		vector<File> Directory::enumerateFiles(std::string const& pattern, bool recursive) const
		{
            this->assertValid();
        ...
        vector<Directory> Directory::enumerateDirectories(std::string const& pattern, bool recursive) const
        {
            this->assertValid();
		...
		void Directory::getFile(std::string const& relativePath, File& buffer) const
		{
			buffer = File(Path::Combine(this->getPath(), relativePath));
		}
		File Directory::getFile(std::string const& relativePath) const
		{
			File rv;
        ...
        Directory Directory::Create(std::string const& path, bool createIntermediates)
        {
            Directory dir(path);
